...
describe("constructDamage", () => {
  ...
  it("should override default damage values with provided arguments", () => {
    const customDamage: Partial<Data_Skill["damage"]> = {
      type: 1,
      elementId: 2,
  ...
  it("should partially override default damage values with provided arguments", () => {
    ...
    expect(result.type).toBe(0); // Default value
    expect(result.elementId).toBe(0); // Default value
    expect(result.variance).toBe(20); // Default value
    expect(result.critical).toBe(false); // Default value
  });
